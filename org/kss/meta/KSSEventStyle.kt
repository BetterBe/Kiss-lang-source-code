...
class KSSEventStyle : KSSNode {
    ...
    val context : KSSParser.EventDeclarationContext
    ...
    val style_affected = ArrayList<KSSNode>()

    val selector : KSSDescendantSelector
    val pointed_selector : KSSDescendantSelector?

    constructor(ctx : KSSParser.EventDeclarationContext){
        context = ctx
        selector = KSSDescendantSelector(ctx.eventHeader().stylingDescendantList())
        if(ctx.eventHeader().eventAddon().pointerAddon()!=null){
            pointed_selector = KSSDescendantSelector(ctx.eventHeader().eventAddon().pointerAddon().stylingDescendantList())
        }else{
            pointed_selector = null
        }
        populateChildren()
    }

    val signals = ArrayList<MetaSignal>()

    fun run(styler : KSSStyler){
        getAffected(styler.variables,styler.affected)
        val eventName = context.eventHeader().eventAddon().name().text
        for(node in affected){
            val event = node.kss_eventList.get(eventName)
            if(event!=null){
                val signal =
                        MetaSignal(fun(args) {
                            //style the style_affected
                            for (variable in variables) {
                                styler.variables.put(variable.key, variable.value)
                                styler.setVariable(variable.key, variable.value)
                            }
                            for (styled_node in style_affected) {
                                for (tag in tag_attributes) {
                                    styled_node.kss_tags.put(tag.key, tag.value)
                                }
                                for (attribute in attributes) {
                                    ...
                                    styler.setAttributeTo(styled_node, attribute.key, attribute.value, numberType, true)
                                }
                                for (attribute in attributes_m) {
                                    ...
                                    for (subAttribute in attribute.value) {
                                        ...
                                        styler.setAttributeTo(styled_node, attribute.key, subAttribute, numberType, c == attribute.value.size)
                                    }
                                }
                            }
    ...
    fun getAffected(vars : HashMap<String,Any?>,nodes : ArrayList<KSSNode>){
        ...
        affected.clear()
        affected.addAll(aff)
        if(pointed_selector!=null) {
            val s_aff = pointed_selector.getAllThatApply(vars,nodes)
            style_affected.clear()
            style_affected.addAll(s_aff)
        }else{
            style_affected.clear()
            style_affected.addAll(affected)
        }
    }
